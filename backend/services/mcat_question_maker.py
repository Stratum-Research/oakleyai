...
Return ONLY valid JSON, no markdown formatting or additional text."""
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                ...
                if response.status_code == 404:
                    ...
                    logger.error(f"OpenRouter API 404 error. Model: {self.model}, Response: {error_detail}")
                    # Check if it's a data policy issue
                    if "data policy" in error_detail.lower() or "privacy" in error_detail.lower():
                        raise ValueError("The selected AI model requires privacy settings to be configured. Please check your OpenRouter account settings or try a different model.")
                    raise ValueError(f"The AI model '{self.model}' is not available. Please check your model configuration.")
                ...
                try:
                    ...
                    pass
                logger.error(f"OpenRouter API error ({e.response.status_code}). Model: {self.model}, Detail: {error_detail or str(e)}")
                
                # Return user-friendly error messages
                if e.response.status_code == 401:
                    raise ValueError("Authentication failed. Please check your API key.")
                elif e.response.status_code == 403:
                    raise ValueError("Access denied. Please check your API key permissions.")
                    ...
                    raise ValueError("Failed to generate questions. Please try again or contact support if the problem persists.")
    ...
    def _parse_correct_answer(self, correct_answer: any, answer_choices: List[str]) -> int:
        """Parse correct answer to integer index (0-3)."""
        # If it's already an integer, validate and return
        if isinstance(correct_answer, int):
            if 0 <= correct_answer <= 3:
                return correct_answer
            raise ValueError(f"correct_answer index {correct_answer} is out of range (0-3)")
        ...
        if isinstance(correct_answer, str):
            ...
            for idx, choice in enumerate(answer_choices):
                ...
                if cleaned_correct_lower == cleaned_choice or cleaned_correct_lower in cleaned_choice or cleaned_choice in cleaned_correct_lower:
                    return idx
        
        # Default to 0 if we can't parse (better than failing)
        logger.warning(f"Could not parse correct_answer '{correct_answer}', defaulting to index 0")
        return 0
    ...
    def _build_questions(
        ...
        for idx, q_data in enumerate(questions_data[:num_questions], 1):
            try:
                ...
                cleaned_choices = [self._clean_answer_choice(str(choice)) for choice in raw_choices]
                
                # Ensure we have exactly 4 choices
                if len(cleaned_choices) != 4:
                    raise ValueError(f"Question {idx} must have exactly 4 answer choices, got {len(cleaned_choices)}")
                
                # Parse correct answer to integer index
                correct_answer_idx = self._parse_correct_answer(
                    q_data.get("correct_answer", 0),
                    cleaned_choices
                )
                ...
                logger.error(f"Error processing question {idx}: {str(e)}. Question data: {q_data}", exc_info=True)
                raise ValueError(f"Failed to process question {idx}. Please try generating questions again.")
        return questions
    ...
    async def generate_questions(
        ...
        logger.info(f"Parsed {len(questions_data)} questions, building Question objects...")
